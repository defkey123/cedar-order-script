...
while True:
    database = firebase.FirebaseApplication(
        'https://cedarrestaurants-ad912.firebaseio.com/Qn4DcOURt9NUPCqctVvnpADZJe83', None)
    orderTTl = ""
    ...
    for i in range(len(orders)):
        ...
        if(orders[orderNames]["translated"] == 0):
            ...
            for i in range(len(cart)):
                ...
                size = "NULL"
                extras = []
                itemPUT = [["title",putValN],["price",putValP]]
                indx = random.randint(1,999999)
                database.put("/Qn4DcOURt9NUPCqctVvnpADZJe83/cart/"+str(orderNames)+"/sortedCart/"+str(indx),itemPUT[0][0],itemPUT[0][1])
                database.put("/Qn4DcOURt9NUPCqctVvnpADZJe83/cart/" + str(orderNames)+"/sortedCart/"+str(indx),itemPUT[1][0],itemPUT[1][1])
                orderTTl += putValN
                orderTTl += "|"
                total += float(putValP)
                ExtraPriceTtl = 0
            ...
            timeZZ = str(now.year) + "," + str(now.month) + "," + str(now.day) + "," + str(now.hour) +","+ str(now.minute) +","+ str(orderNames)
            database.put("/Qn4DcOURt9NUPCqctVvnpADZJe83/log" ,timeZZ, [orderTTl,taxTotal])
            time.sleep(0.005)